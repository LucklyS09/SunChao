...
public class ClaszDAO extends BaseHibernateDAO {
	...
	private void addStudent(int claszId,Student stud){
	...
	private void addStudent(Clasz cls,Student stud){
		Session session=getSession();
		cls.getStudents().add(stud);
		Transaction trans=session.beginTransaction();
		session.save(cls);
	...
	public static void main(String[] args) {
		ClaszDAO md=new ClaszDAO();
//		Clasz master=new Clasz();
//		master.setName("oooo");
//		Student s=new Student();
//		s.setName("s11s");
//		md.addStudent(master, s);
		md.testFetch();
